...
def gp_student_view(request):
    if request.user.is_staff:
        ...
        pollusers = [pu for pu in PollUser.objects.order_by('entry_year').all() if is_gp_student=True]
        return render(request, template, {'pollusers':pollusers})
    else:
        return HttpResponseRedirect(reverse('polls:home'))
...
def ngp_student_view(request):
    if request.user.is_staff:
        ...
        pollusers = [pu for pu in PollUser.objects.order_by('entry_year').all() if is_gp_student=False]
        return render(request, template, {'pollusers':pollusers})
    else:
        return HttpResponseRedirect(reverse('polls:home'))
